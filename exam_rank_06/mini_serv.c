...
void	add_client(t_client **lst, int fd, int id)
{
	t_client *new = 0;
...
void	remove_client(t_client **lst, int fd)
{
	t_client *tmp = *lst;
	t_client *prev = 0;
...
void	send_all(t_client *lst, char *str, int fd, fd_set *writefds)
{
	for (; lst; lst = lst->next)
		if (FD_ISSET(lst->fd, writefds) && lst->fd != fd)
...
int		main(int ac, char **av)
{
	...
	while (1)
	{
		init_set(client, &readfds);
		writefds = readfds;
		select(FD_SETSIZE, &readfds, &writefds, 0, 0);

		if (FD_ISSET(sockfd, &readfds))
		{
			if ((connfd = accept(sockfd, (struct sockaddr *)&cli, &len)) < 0)
				fatal_error("Fatal error\n");
			fcntl(connfd, F_SETFL, O_NONBLOCK); // DA ELIMINARE ALLA CONSEGNA
			bzero(str, sizeof(str));
			add_client(&client, connfd, g_id++);
			sprintf(str, "server: client %d just arrived\n", g_id - 1);
			send_all(client, str, connfd, &writefds);
		}
			...
			if (FD_ISSET(connfd, &readfds))
			{
				...
				else if (i > 0)
				{
					...
					while ( extract_message(&msg, &extr) )
					{
						...
						send_all(client, tmp_str, connfd, &writefds);
						free(extr);
						free(tmp_str);
						extr = 0;
						tmp_str = 0;
